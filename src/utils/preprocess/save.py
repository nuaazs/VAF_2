...
def save_file(file, spk, channel, upload=False):
    ...
    if cfg.ONLY_USE_ENERGY_VAD:
        run_cmd(f"ffmpeg -i {save_path} -y -ss {cfg.WAV_START} -to {end} -f s16le -acodec pcm_s16le -ar {cfg.SR} -ac 1 -vn -map_channel 0.0.{channel} -map_metadata -1 {save_path_wav}")
    else:
