...
do
    ...
    docker run --name vaf_${i} --gpus "device=${cuda_num}" -v ${PWD}/cpp_${i}:/VAF/src/cpp -v ${PWD}/cfg_${i}.py:/VAF/src/cfg.py -v ${PWD}/log_${i}:/VAF/src/log -v ${PWD}/pretrained_models_${i}:/VAF/src/pretrained_models --net host zhaosheng/vaf:v1.0 > ${i}_docker.log 2>&1 &
done
