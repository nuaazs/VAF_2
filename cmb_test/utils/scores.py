...
def calc_result(scores_list,labels_list,p_target,c_miss,c_fa,min_recall,min_precision):
    ...
    min_dcf = compute_c_norm(fnr,
                            ...
                            c_miss=c_miss,
                            c_fa=c_fa)
    th_matrix_result = compute_tn_fn_tp_fp(scores, labels,min_recall=min_recall,min_precision=min_precision)
    # write the metrics
    print("\t\tEER = {0:.4f}".format(100 * eer))
    print("\t\tminDCF (p_target:{} c_miss:{} c_fa:{}) = {:.4f}".format(
    ...
    for _info in th_matrix_result:
        ...
        try:
            ...
            precision = " - "
            recall = " - "
        print(f"\t\tTH:{th:.2f}\tTP:{tp:.4f}\tFP:{fp:.4f}\tTN:{tn:.4f}\tFN:{fn:.4f}\tP:{precision:.2f}\tR:{recall:.2f}\tACC:{acc:.2f}")
