...
def add_hit(pipeline_result):
    ...
    try:
        ...
        hit_score = pipeline_result["compare_result"]['ERES2NET_Base']["score"]
        hit_spkid = pipeline_result["compare_result"]['ERES2NET_Base']["hit_spkid"]
...
def get_hit_info(page_no, page_size):
    ...
    try:
        page_no = (page_no - 1) * page_size
        query_sql = f"select * from hit_ppt order by hit_time desc limit {page_no},{page_size};"
        cursor.execute(query_sql)
        result = cursor.fetchall()
...
def get_table_info():
    ...
    try:
        ...
        cursor.execute(query_sql)
        result['today_count'] = cursor.fetchone()['count(*)']
        query_sql = f"select count(*) from black_speaker_info_ppt;"
        cursor.execute(query_sql)
        result['black_spk_info'] = cursor.fetchone()['count(*)']
        query_sql = f"select count(*) from hit_ppt;"
        cursor.execute(query_sql)
        result['hit_count'] = cursor.fetchone()['count(*)']
        # random 97.0-99.9
