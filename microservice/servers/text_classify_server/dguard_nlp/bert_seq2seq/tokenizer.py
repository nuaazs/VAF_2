...
class Tokenizer(object):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        if not os.path.exists(resolved_vocab_file):    # 如果没有这个vocab文件， 那我们就要下载
            if pretrained_model_name_or_path in VOCAB_ARCHIVE_URLS_MAP:
                ...
                f = requests.get(url)
                os.makedirs(os.path.dirname(resolved_vocab_file), exist_ok=True)
    ...
    def prepare_for_model(
        ...
        if truncation is True and (max_length is not None and total_len > max_length):
        ...
        if add_special_tokens:
            if pair_ids is not None:
                sequence = [self.token_start_id] + ids + [self.token_end_id] + pair_ids + [self.token_end_id]
    ...
    def encode_plus(
        ...
        second_ids = get_input_ids(second_text) if second_text is not None else None
...
class T5PegasusTokenizer(Tokenizer):
    def __init__(self, vocab_path, pre_tokenizer=lambda x: jieba.cut(x, HMM=False), **kwargs):
    ...
    def tokenize(self, text, *arg, **kwargs):
        ...
        for text in self.pre_tokenizer(text):
            if text in self.vocab:
                ...
                split_tokens.extend(super().tokenize(text))
        return split_tokens
    ...
    def encode_plus(
        ...
        return super().encode_plus(text, second_text, add_special_tokens, truncation, max_length)