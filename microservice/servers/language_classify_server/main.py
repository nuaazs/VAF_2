...
def main():
    try:
        ...
        logger.info(f"# spkid: {spkid} channel: {channel} start: {start} length: {length} save_oss: {save_oss} filelist: {filelist} score_threshold: {score_threshold}")
        ...
        for fileurl in filelist:
            try:
                ...
                err_logger.error(f"File download failed: {str(e)}")
