...
def fsmn_vad(filepath, save_folder_path, smooth_threshold=0.5, min_duration=1, save_oss=False):
    ...
    timelist = [[float(_[0]/1000),float(_[1]/1000)] for _ in timelist]
...
@app.route('/fsmn_vad/<filetype>', methods=['POST'])
def main(filetype):
    try:
        # try:
        spkid = request.form.get('spkid')
        channel = int(request.form.get('channel', 0))
        smooth_threshold = float(request.form.get('smooth_threshold', 1))
