...
class add_model(nn.Module):
    def __init__(self, model, model_add):
        ...
        # Initialize the convolution and fully connected layers without specifying the in/out features
        # These will be dynamically set in the forward function
        self.conv1x1 = None
        self.fc = None
    ...
    def forward(self, x):
        ...
        x2_features = x2.size(1)  # Assuming the feature dimension is the second dimension (Batch x Channel x H x W)
        x1_features = x1.size(1)
        if self.conv1x1 is None or self.conv1x1.in_channels != x2_features or self.conv1x1.out_channels != x1_features:
            self.conv1x1 = nn.Conv2d(x2_features, x1_features, kernel_size=1).to(x.device)

        x2 = self.conv1x1(x2)

        # Concatenate x1 and x2 along the feature dimension
        combined = torch.cat([x1, x2], dim=1)  # Assuming feature dimension is 1

        # Dynamically set the fully connected layer to match the concatenated features size to x1's
        combined_features = combined.size(1) * combined.size(2) * combined.size(3)  # Channel x H x W
        if self.fc is None or self.fc.in_features != combined_features or self.fc.out_features != x1_features * x1.size(2) * x1.size(3):
            self.fc = nn.Linear(combined_features, x1_features * x1.size(2) * x1.size(3)).to(x.device)

        # Flatten and pass through the fully connected layer
        combined = combined.view(combined.size(0), -1)
        combined = self.fc(combined)

        # Reshape to the original x1 shape and add the residual connection
...
def train(config='conf/config.yaml', **kwargs):
    ...
    model_add = ECAPA_TDNN(input_size=80,
