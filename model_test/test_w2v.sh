...
extract_embeddings() {
    ...
    if [ ! -d result/$exp_name/$model/$trial/embeddings ] || $overwrite; then
        torchrun --nproc_per_node=$nj --master_port=$master_port extract_test_emb_wav2vec2.py --seed $seed --length $length --model_name $model --exp_dir result/$exp_name/$model/$dataset/$length \
