...
parser.add_argument('--fold_path', type=str, default='/datasets_hdd/testdata_cnceleb_16k/',help='Folder of raw wav files')
parser.add_argument('--dst_path', type=str, default='/datasets_hdd/testdata_cnceleb_16k_vad/',help='Folder to save vad wav files')
...
if __name__ == "__main__":
    ...
    all_wavs = []
    for mf in os.listdir(args.fold_path):
        mf_path = os.path.join(args.fold_path,mf)
        #print(tr_path)
        for tr in os.listdir(mf_path):
            tr_path=os.path.join(mf_path,tr)
            #print(id_path)
            for id in os.listdir(tr_path):
                id_path=os.path.join(tr_path,id)
                for file in os.listdir(id_path):
                    if file.endswith(".wav"):
                        all_wavs.append(os.path.join(id_path, file))
    print(all_wavs)
