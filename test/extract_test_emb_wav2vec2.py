...
def wav2vec2_feature_extractor(wav):

    feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("/home/duanyibo/dyb/chinese-wav2vec2-large-fairseq-ckpt.pt")
    model = Wav2Vec2Model.from_pretrained("/home/duanyibo/dyb/pytorch_model.bin")
            ...
            with torch.no_grad():
                with WriteHelper(f'ark,scp:{emb_ark},{emb_scp}') as writer:
                    for k in tqdm(local_k):
                        ...
                        feat = wav2vec2_feature_extractor(wav)
        ...
        with torch.no_grad():
            with WriteHelper(f'ark,scp:{emb_ark},{emb_scp}') as writer:
                for k in tqdm(local_k):
                    ...
                    wav_a = wav[:,:half_len]
                    wav_b = wav[:,half_len:]
                    feat_a = wav2vec2_feature_extractor(wav_a)
                    feat_b = wav2vec2_feature_extractor(wav_b)
