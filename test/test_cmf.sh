        ...
        for trial_class in $trials_class; do
                if [ "$trial_class" == "voxceleb" ]; then
                        ...
                        if [ ! -d $result_path/$model_id/voxceleb_result/embeddings ]; then
                                torchrun --nproc_per_node=$nj  --master_port=35641 speakerlabduanyibo/bin/extract_cmf.py --exp_dir $result_path/$model_id/voxceleb_result \
                                                                --data $vox_scp --use_gpu --gpu $gpus || wechat echo "voxceleb torchrun error"
                                mkdir -p $result_path/$model_id/voxceleb_result
                        fi
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/voxceleb_result/embeddings --test_data $result_path/$model_id/voxceleb_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/voxceleb_result/scores --trials $trials_vox || wechat echo "voxceleb compute_score_metrics_with_CMF error"
                fi

                if [ "$trial_class" == 'cnceleb' ]; then
                        echo "cnceleb dataset"
                        if [ ! -d $result_path/$model_id/cnceleb_result/embeddings ]; then
                                torchrun --nproc_per_node=$nj --master_port=45642 speakerlabduanyibo/bin/extract_cmf.py --exp_dir $result_path/$model_id/cnceleb_result \
                                                                --data $cnceleb_scp --use_gpu --gpu $gpus || wechat echo "cnceleb torchrun error"
                                mkdir -p $result_path/$model_id/cnceleb_result 
                        fi
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/cnceleb_result/embeddings --test_data $result_path/$model_id/cnceleb_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/cnceleb_result/scores --trials $trials_cnceleb || wechat echo "cnceleb compute_score_metrics_with_CMF error"
                fi

                if [ "$trial_class" == 'cti' ]; then
                        echo "cti dataset"
                        if [ ! -d $result_path/$model_id/cti_result/embeddings ]; then
                                torchrun --nproc_per_node=$nj --master_port=55643 speakerlabduanyibo/bin/extract_cmf.py --exp_dir $result_path/$model_id/cti_result \
                                                                --data $cti_scp --use_gpu --gpu $gpus || wechat echo "cti torchrun error"
                                mkdir -p $result_path/$model_id/cti_result
                        fi
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/cti_result/embeddings --test_data $result_path/$model_id/cti_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/cti_result/scores --trials $trials_cti || wechat echo "cti compute_score_metrics_with_CMF error"
                fi

                if [ "$trial_class" == '3dspeaker' ]; then
                        echo "3dspeaker dataset"
                        if [ ! -d $result_path/$model_id/3dspeaker_result/embeddings ]; then
                                torchrun --nproc_per_node=$nj --master_port=55688 speakerlabduanyibo/bin/extract_cmf.py --exp_dir $result_path/$model_id/3dspeaker_result \
                                                                --data $speaker_scp --use_gpu --gpu $gpus || wechat echo "3d torchrun error"
                                mkdir -p $result_path/$model_id/3dspeaker_result
                        fi
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/3dspeaker_result/embeddings --test_data $result_path/$model_id/3dspeaker_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/3dspeaker_result/scores --trials $trials_3dspeaker || wechat echo "3d compute_score_metrics_with_CMF error"
                fi

                if [ "$trial_class" == 'male' ]; then
                        echo "male dataset"
                        if [ ! -d $result_path/$model_id/male/embeddings ]; then
                                torchrun --nproc_per_node=$nj --master_port=54688 speakerlabduanyibo/bin/extract_cmf.py --exp_dir $result_path/$model_id/male_result \
                                                                --data $male_scp --use_gpu --gpu $gpus || wechat echo "cti_male torchrun error"
                                mkdir -p $result_path/$model_id/male_result
                        fi
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/male_result/embeddings --test_data $result_path/$model_id/male_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/male_result/scores --trials $trials_male || wechat echo "cti_male compute_score_metrics_with_CMF error"
                fi  

                if [ "$trial_class" == 'female' ]; then
                        echo "female dataset"
                        if [ ! -d $result_path/$model_id/female/embeddings ]; then
                                torchrun --nproc_per_node=$nj --master_port=53688 speakerlabduanyibo/bin/extract_cmf.py --exp_dir $result_path/$model_id/female_result \
                                                                --data $female_scp --use_gpu --gpu $gpus || wechat echo "cti_female torchrun error"
                                mkdir -p $result_path/$model_id/female_result
                        fi
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/female_result/embeddings --test_data $result_path/$model_id/female_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/female_result/scores --trials $trials_female || wechat echo "female compute_score_metrics_with_CMF error"
                fi 
                ...
                if [ "$trial_class" == 'cti2' ]; then
                        ...
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/cti2_result/embeddings --test_data $result_path/$model_id/cti2_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/cti2_result/scores_male --trials $trials_cti2 || wechat echo "cti2 compute_score_metrics_with_CMF error"
                fi 
                
                if [ "$trial_class" == 'cti_20s' ]; then
                        echo "cti_20s dataset"
                        if [ ! -d $result_path/$model_id/cti_20s/embeddings ]; then
                                torchrun --nproc_per_node=$nj --master_port=53688 speakerlabduanyibo/bin/extract_cmf.py --exp_dir $result_path/$model_id/cti_20s_result \
                                                                --data $cti2_scp_20s --use_gpu --gpu $gpus || wechat echo "cti_20s torchrun error"
                                mkdir -p $result_path/$model_id/cti_20s_result
                        fi
                        python speakerlabduanyibo/bin/compute_score_metrics_with_CMF.py --enrol_data $result_path/$model_id/cti_20s_result/embeddings --test_data $result_path/$model_id/cti_20s_result/embeddings \
                                                                        --scores_dir $result_path/$model_id/cti_20s_result/scores_male --trials $trials_cti2_20s || wechat echo "cti_20s compute_score_metrics_with_CMF error"
                fi 
        done
