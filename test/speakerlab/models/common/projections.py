...
#               2023 ShengZhao (zhaosheng@nuaa.edu.cn)
...
class SphereProduct(nn.Module):
    ...
    def __init__(self, in_features, out_features, margin=2):
        ...
        # duplication formula
        self.m_lambda = [
            _lambda x: x**0, _lambda x: x**1, _lambda x: 2 * x**2 - 1,
            _lambda x: 4 * x**3 - 3 * x, _lambda x: 8 * x**4 - 8 * x**2 + 1,
            _lambda x: 16 * x**5 - 20 * x**3 + 5 * x
        ]
        assert self.margin < 6

    def forward(self, input, label):
        # _lambda = max(_lambda_min,base*(1+gamma*iteration)^(-power))
        self.iter += 1
        self.lamb = max(
            self._lambdaMin,
        ...
        cos_m_theta = self.m_lambda[self.margin](cos_theta)
