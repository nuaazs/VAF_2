...
data=data
exp=exp
exp_name=cjsd8000_resnet293
data_type=raw #shard
num_avg=10
gpus=[2,3,4,5,6,7]
checkpoint=/VAF/train/egs/cti/sv-eres2net/exp/cjsd8000_resnet293/models/model_175.pt
config=/VAF/train/egs/cti/sv-eres2net/conf/eres2net34_aug_wenet.yaml
lm_config=/VAF/train/egs/cti/sv-eres2net/conf/eres2net34_aug_wenet_lm.yaml
trials="cjsd1000_normal.kaldi"
score_norm_method="asnorm"  # asnorm/snorm
top_n=300
exp_dir=$exp/$exp_name
echo "Startinng parsing..."
. tools/parse_options.sh || exit 1
echo "Parsed."

if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  echo "Stage 1: Preparing dataset ..."
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  for dset in cjsd8000 cjsd1000; do
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --standalone --nnodes=1 --nproc_per_node=$num_gpus \
    dguard/bin/wenet_trainer.py --config $config \
      ...
      --num_avg ${num_avg} \
      --data_type "${data_type}" \
      --train_data ${data}/cjsd8000/${data_type}.list \
      --train_label ${data}/cjsd8000/utt2spk \
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  python dguard/wespeaker/bin/export_jit.py \
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
  ...
  echo "lm config: ${lm_config}"
  echo "NCCL_IGNORE_DISABLED_P2P=1 bash run.sh --stage 3 --stop_stage 7 \
      --data ${data} \
      --data_type ${data_type} \
      --config ${lm_config} \
      --exp_dir ${lm_exp_dir} \
      ...
      --score_norm_method ${score_norm_method} \
      --top_n ${top_n}"
  NCCL_IGNORE_DISABLED_P2P=1 bash run.sh --stage 3 --stop_stage 7 \
      --data ${data} \
      --data_type ${data_type} \
      --config ${lm_config} \
      --exp_dir ${lm_exp_dir} \
