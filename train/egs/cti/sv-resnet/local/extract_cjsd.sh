...
nj=4
gpus="[0,1]"
data_type="raw" #"shard"  # shard/raw/feat
data=data

. tools/parse_options.sh
set -e

data_name_array=("cjsd8000" "cjsd1000")
data_list_path_array=("${data}/cjsd8000/${data_type}.list" "${data}/cjsd1000/${data_type}.list")
data_scp_path_array=("${data}/cjsd8000/wav.scp" "${data}/cjsd1000/wav.scp") # to count the number of wavs
nj_array=($nj $nj)
...
for i in $(seq 0 $(($count - 1))); do
  ...
  echo ${data_scp_path_array[$i]}
  echo "WAV nums: ${wavs_num}"
