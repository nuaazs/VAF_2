...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  torchrun --nproc_per_node=$num_gpu dguard/bin/train_rdino.py --config conf/rdino.yaml --gpu $gpus \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  nj=4
  torchrun --nproc_per_node=$nj dguard/bin/extract_rdino.py --exp_dir $exp_dir \
