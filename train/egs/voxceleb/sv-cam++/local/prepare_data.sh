...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  # vox1
  echo "Prepare vox1 wav.scp, utt2spk, spk2utt, and trials ..."
  echo "Looking for vox1 wav files in ${rawdata_dir}/voxceleb1 ..."
  real_voxceleb1_path=`realpath ${rawdata_dir}/voxceleb1`
  find ${real_voxceleb1_path} -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' | sort >${data}/vox1/wav.scp
  ...
  # vox2
  echo "Prepare vox2_dev wav.scp, utt2spk, and spk2utt ..."
  echo "Looking for vox2_dev wav files in ${rawdata_dir}/voxceleb2_wav ..."
  real_voxceleb2_wav_path=`realpath ${rawdata_dir}/voxceleb2_wav`
  find ${real_voxceleb2_wav_path} -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' | sort >${data}/vox2_dev/wav.scp
