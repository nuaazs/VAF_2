...
cohort_set=vox2_dev
top_n=100
exp_dir="/VAF/train/egs/voxceleb/sv-repvgg/exp/repvgg"
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  python dguard/tools/vector_mean.py \
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  for x in $trials; do
    python dguard/bin/score_norm.py \
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  ...
  for x in ${trials}; do
    ...
    python dguard/bin/compute_metrics.py \
      --p_target 0.01 \
      --c_fa 1 \
      --c_miss 1 \
    ...
    python dguard/bin/compute_det.py \
      ${scores_dir}/${output_name}_${x}.score
  done
fi